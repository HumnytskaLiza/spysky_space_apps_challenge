<template>
  <div id="map-container">
    ...
    <div id="map-panels">
      ...
      <Filters
        @id-was-selected="getSelectedId"
        @name-was-selected="getSelectedName"
        @time-was-selected="getSelectedTime"
        @filters-changed="filterOjects"
      />
    </div>
...
textureLoader.load("src/assets/background-textures/bg_map.jpg", (texture) => {
  ...
  const geometry = new THREE.SphereGeometry(1000, 10, 10);
...
async function getAsteroids() {
  try {
    const res = await axios.get(`${url}/iridium/`);
...
function filterOjects() {
  satellitesGroup.forEach((s) => {
    ...
    if (
      s_date >= new Date(filters["time"][0]) &&
      s_date <= new Date(filters["time"][1])
    ) {
      s.visible = true;
...
onMounted(() => {
  ...
  getAsteroids().then(() => {
    if (asteroids.value.length > 0) {
      asteroids.value.forEach((s) => {
        addAsteroids(s.x, s.y, s.z);
      });
    }
  });
